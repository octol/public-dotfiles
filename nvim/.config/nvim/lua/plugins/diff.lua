...
local function against_base(template)
  return function()
    ...
    local base = rs.code == 0 and vim.trim(res.stdout or "") or ""
    if base == "" then
...
-- The explorer only has global `focus_on_select`, so <CR> either always follows
-- the file into the iff or never does. Flip the option around one select to get
...
return {
  ...
  -- The explore's mappings are buffer-local and installed by he plugin itself,
  ...
  keys = {
    ...
    -- `...` is git merge-base synax, so these show only what this branch
    ...
    -- between the two mappins below. In the shell, `dr` is
